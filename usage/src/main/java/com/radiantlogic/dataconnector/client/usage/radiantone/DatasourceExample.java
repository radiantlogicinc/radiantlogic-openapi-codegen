...
public class DatasourceExample {
  public static void main(final String[] args) throws Exception {
    ...
    final ApiClient apiClient =
        new ApiClientProvider().createAndPrepareApiClient(basePath, username, password);
    ...
    ldapDataSource.setCategory(
        com.radiantlogic.custom.dataconnector.radiantonev8api.model.RequiredDataSourceCategory
            .LDAP);
    ldapDataSource.setType("Active Directory");
    ldapDataSource.setName("mydatasource");

    dataSourcesApi.createDataSource(ldapDataSource);
    final LdapDataSource result =
        (LdapDataSource) dataSourcesApi.getDataSource(ldapDataSource.getName());
  }
}
